 ...
 */
final class StrictTsdbQuery implements Query {
  ...
  /** Constructor. */
  public StrictTsdbQuery(final TSDB tsdb) {
    this.tsdb = tsdb;
  }
  ...
  Scanner getScanner() throws HBaseException {
    ...
    Bytes.setInt(end_row, (int) getScanStartTime(), metric_width);
    System.arraycopy(metric, 0, start_row, 0, metric_width);
    System.arraycopy(metric, 0, end_row, 0, metric_width);
    int idx = metric_width + Const.TIMESTAMP_BYTES;
    for (final byte[] tag : tags) {
      System.arraycopy(tag, 0, start_row, idx, tagsize);
      System.arraycopy(tag, 0, end_row, idx, tagsize);
      idx += tagsize;
    }
    start_row[start_row.length - 1] = (byte)0;
    end_row[end_row.length - 1] = (byte)0xFF;
